...
void write_message(int m, TreeNode *p, LinkNode *head, HuffCode &HC,char *compressTXT, char *compressedPath) {
    char s[maxmum] = {'\0'};
    char c;
    ...
    ifstream in_out;
    //打开输出文件错误
    char *compressedConfig = (char *) malloc(strlen(compressedPath) + strlen("\\config.txt"));
    sprintf(compressedConfig, "%s%s", compressedPath, "\\config.txt");
    char *compressedOnly0And1 = (char *) malloc(strlen(compressedPath) + strlen("\\config.txt"));
    sprintf(compressedOnly0And1, "%s%s", compressedPath, "\\Only0And1.txt");
    write_out.open(compressedConfig);
    write1_out.open(compressedOnly0And1);
    in_out.open(compressTXT);
    if (write_out.fail()) {
...
//    write_out << m;
    // cout<<m<<endl;
    write_out.write(reinterpret_cast<char *>(&m),sizeof(m));
    int i = m;
